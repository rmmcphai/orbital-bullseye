...
class VehicleIndexMap final {
 public:
  ...
  static constexpr std::size_t capacity() noexcept { return MAX_VEHICLES; }
  ...
  bool contains(VehicleId id) const noexcept { return index_of(id).has_value(); }
 ...
 private:
  // NOTE: This assumes MAX_VEHICLES exists in your project. If it lives elsewhere,
  // include the appropriate header instead of relying on it being globally visible.
  static constexpr std::size_t MAX_VEHICLES = 32;
