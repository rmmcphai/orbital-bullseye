...
TimeGrid make_time_grid(double horizon_sec, double cadence_sec) {
  static auto log = bullseye::logging::get(bullseye::logname::kCoreTimeGrid);
  ...
  LOG_DEBUGF(log, "grid horizon=%.17g cadence=%.17g steps=%zu last=%.17g",
             horizon_sec, cadence_sec, grid.tau.size(),
             grid.tau.empty() ? 0.0 : grid.tau.back());
