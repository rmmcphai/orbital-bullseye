...
[[nodiscard]] inline double norm(const Vec3& v) noexcept
{
...
struct Mat3 final
{
    ...
    [[nodiscard]] constexpr double operator()(int r, int c) const noexcept
    {
        return m[r][c];
    }
    constexpr double& operator()(int r, int c) noexcept
    {
        return m[r][c];
    }
};
