...
inline double center_bound(const Vec3& chief_r_i, double abs_m, double rel) noexcept
{
                                    ...
                                    const FrameValidationTolerances& tol) noexcept
{
    ...
    if (!std::isfinite(t0) || !std::isfinite(chief.time_tag) || !std::isfinite(frame.time_tag) ||
        !finite_vec(chief.r_i) || !finite_vec(chief.v_i) || !finite_vec(frame.origin_i) ||
        !finite_mat(frame.C_from_ric_to_inertial))
    {
        out.status.code = ProviderCode::kInvalidInput;
    ...
    // Time tag policy (FR-14a + GCR-5 tolrtance).
    // If contracts::Tol::kAdoptedTimeTag_sec == 0 -> exact match required.
    const double dt_tol = contracts::Tol::kAdoptedTimeTag_sec;
    const bool time_ok = (dt_tol == 0.0) ? (frame.time_tag == t0)
                                               : (std::fabs(frame.time_tag - t0) <= dt_tol);
    if (!time_ok)
    {
    ...
    if(contracts::Adopted::kRequireFrameKindDeclaration)
    {
        if (frame.frame_kind != contracts::Adopted::kRequiredFrameKind ||
            frame.axis_order != contracts::Adopted::kRequiredAxisOrder)
        {
            out.status.code = ProviderCode::kInvalidInput;
