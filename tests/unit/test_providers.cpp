...
TEST_CASE("CartesianChiefProvider (current) returns exact-time sample (FR-14)", "[providers]") {
  ...
  CartesianChiefProvider p{kFrameId, CartesianChiefProvider::Mode::kCurrent, /*warn_period_sec=*/0.0};
...
TEST_CASE("CartesianChiefProvider (timeseries) returns exact-time sample (FR-14)", "[providers]") {
  constexpr const char* kFrameId = "INERTIAL";
  CartesianChiefProvider p{kFrameId, CartesianChiefProvider::Mode::kTimeSeries, /*warn_period_sec=*/0.0};
...
TEST_CASE("CartesianChiefProvider is deterministic for identical inputs", "[providers][determinism]") {
  constexpr const char* kFrameId = "INERTIAL";
  CartesianChiefProvider p{kFrameId, CartesianChiefProvider::Mode::kTimeSeries, /*warn_period_sec=*/0.0};
...
TEST_CASE("CartesianChiefProvider returns kInvalidInput when inertial frame id is null", "[providers]") {
  CartesianChiefProvider p{nullptr, CartesianChiefProvider::Mode::kCurrent, /*warn_period_sec=*/0.0};
...
TEST_CASE("TwoBodyChiefProvider returns deterministic state for same t0", "[providers][twobody][determinism]") {
  // Earth mu (SI).
  ...
  // Bitwise-ish checks (exact equality) are acceptable here because we run the same code path twice.
...
TEST_CASE("TwoBodyChiefProvider produces finite outputs and varies with time", "[providers][twobody]") {
  const double mu = 3.986004418e14;
  const Vec3 r0{7000e3, 0.0, 0.0};
  const Vec3 v0{0.0, 7546.05329, 0.0};
